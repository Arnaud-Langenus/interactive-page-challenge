    ...
    function moveCarouselTo(slide) {  // Check if carousel is moving, if not, allow interaction
        if(!moving) {    // temporarily disable interactivity
            ...
            if ((totalItems - 1) > 3) {      // Checks and updates if the new slides are out of bounds
                ...
                itemsArray[oldPrevious].className = images;
                itemsArray[oldNext].className = images;      // Add new classes
                itemsArray[newPrevious].className = images + " prev";
                itemsArray[slide].className = images + " active";
                itemsArray[newNext].className = images + " next";
            }
        }
    }
